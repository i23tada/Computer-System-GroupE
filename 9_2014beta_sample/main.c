...
unsigned int zero_pattern[8]={
  0,0,0,0,0,0,0,0
};

unsigned int gu_pattern_green[8]={
  0x0000,0x6e00,0x6e00,0x6e00,0x6e00,0x0e00,0x3e00,0x0000
};

unsigned int choki_pattern_green[8]={
  0x0000,0x0000,0x7e00,0x7e00,0x7000,0x7e00,0x3e00,0x0000
};

unsigned int pa_pattern_green[8]={
  0x0000,0x3e00,0x7e00,0x7e00,0x7e00,0x7e00,0x3000,0x0000
};

unsigned int gu_pattern_red[8]={
  0x0000,0x006e,0x006e,0x006e,0x006e,0x000e,0x003e,0x0000
};

unsigned int choki_pattern_red[8]={
  0x0000,0x0000,0x007e,0x007e,0x0070,0x007e,0x003e,0x0000
};

unsigned int pa_pattern_red[8]={
  0x0000,0x003e,0x007e,0x007e,0x007e,0x007e,0x0030,0x0000
};
...
void matrix_change(int hand){
  switch(hand){
      case gu:
        matrix_set(gu_pattern_green);
        break;
      case choki:
        matrix_set(choki_pattern_green);
        break;
      case pa:
        matrix_set(pa_pattern_green);
        break;
    }
}
...
void do_mode5(UI_DATA* ud){
  ...
  if(result_flag==TRUE){
    ...
    if(hand_flag==result){
      ...
      (hand_flag==gu    && result==choki) ||
      (hand_flag==choki && result==pa)    ||
  ...
  case KEY_LONG_C:
    matrix_set(zero_pattern);
    ud->mode=MODE_0;
    break;

  case KEY_SHORT_L:
    if(!pouse_flag && !result_flag){
      if(sel_arr>SEL_RANG_MIN)
        sel_arr-=5;
  ...
  default:
    if(!pouse_flag && !result_flag){
      ...
      switch(sel_arr){
        case 1:
          matrix_set(gu_pattern_red);
          break;
        case 6:
          matrix_set(choki_pattern_red);
          break;
        case 11:
          matrix_set(pa_pattern_red);
          break;
      }
